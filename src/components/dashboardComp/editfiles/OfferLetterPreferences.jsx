...
import { CountrySelect, InstituteComponent, SelectComponent } from "../../reusable/Input";
...
const OfferLetterPreferences = ({appId, updatedData}) => {
    ...
    if (applicationDataById) {
      setOfferLater({
        preferences: {
          ...
          offerLetterPrice: applicationDataById?.preferences?.offerLetterPrice || "",
  ...
  {isOne && (
    <div className="bg-white rounded-xl py-4 pb-12 mt-6">
      ...
      {errors.prefInstitution && (
        <p className="text-red-500 mt-1 text-sm">{errors.prefInstitution}</p>
      )}
